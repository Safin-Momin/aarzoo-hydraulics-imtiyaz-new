...
<template>
  <Popover>
    <template #default="{ open }">
      <PopoverButton class="relative z-10 flex h-8 w-8 items-center justify-center ring-0 outline-none"
      ...
      <Transition enter-active-class="transition duration-150 ease-out transform" enter-from-class="opacity-0 scale-95"
        enter-to-class="opacity-100 scale-100" leave-active-class="transition duration-100 ease-in transform"
        leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-95">
        <PopoverPanel v-if="open"
          ...
          <div class="space-y-1 p-2">
            <NuxtLink v-for="page in navigation.pages" :key="page.name" :href="page.href" :title="getTitle(page)"
              class="flex-none group relative block transition duration-300 px-3 py-2.5 hover:text-primary-600 text-center"
              :class="[page.activeMenu === currentActiveMenu ? 'text-primary-600 hover:text-primary-400' : 'text-secondary hover:text-primary-600']"
